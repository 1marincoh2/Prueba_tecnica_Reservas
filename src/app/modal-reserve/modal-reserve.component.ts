  ...
  saveReserve() {
    ...
    this._reserveSevice.postReserve(payloadReserve).subscribe({
     ...
     isHoraSelected(hour: string): boolean {
      const reservasEnFecha = this.reservas.filter(reserva => reserva.pedido.fecha_entrega === this.date);
      return reservasEnFecha.some(reserva => reserva.pedido.hora_entrega === hour);
    }
  ngOnInit(): void {
